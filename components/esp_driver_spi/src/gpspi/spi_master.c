...
***MODIFIED FOR POLLING-ONLY MODE***
This driver has been modified to operate in polling mode only. All interrupt-related functionality has been disabled.

A device works in polling mode only:

Work in the polling mode in the task.
   ...
   Alternatively, ``spi_device_transmit`` or ``spi_device_queue_trans`` can be used
   for immediate transaction execution in polling mode.
...
static SPI_MASTER_ISR_ATTR esp_err_t check_trans_valid(spi_device_handle_t handle, spi_transaction_t *trans_desc)
{
    ...
    // DMA-related check removed for polling-only mode
...
esp_err_t SPI_MASTER_ATTR spi_device_get_trans_result(spi_device_handle_t handle, spi_transaction_t **trans_desc, TickType_t ticks_to_wait)
{
    ...
    // In polling-only mode, transactions are executed synchronously
    // This function is not needed since spi_device_queue_trans() already completes the transaction
    ESP_LOGW(SPI_TAG, "spi_device_get_trans_result not needed in polling-only mode. Use spi_device_transmit() instead.");
