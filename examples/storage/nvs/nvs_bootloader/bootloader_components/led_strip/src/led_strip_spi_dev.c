...
// Define maximum LED strip size for static allocation
#define LED_STRIP_MAX_LEDS 256
#define LED_STRIP_MAX_BYTES_PER_PIXEL 4 // RGBW format
#define LED_STRIP_STATIC_BUFFER_SIZE (LED_STRIP_MAX_LEDS * LED_STRIP_MAX_BYTES_PER_PIXEL * SPI_BYTES_PER_COLOR_BYTE)

#define LED_STRIP_SPI_ATTR IRAM_ATTR
...
typedef struct {
    ...
    uint8_t pixel_buf[LED_STRIP_STATIC_BUFFER_SIZE];
} led_strip_spi_obj;

// Static instance for LED strip
static led_strip_spi_obj static_spi_strip;

// please make sure to zero-initialize the buf before calling this function
static void LED_STRIP_SPI_ATTR __led_strip_spi_bit(uint8_t data, uint8_t *buf)
{
...
static esp_err_t LED_STRIP_SPI_ATTR led_strip_spi_set_pixel(led_strip_t *strip, uint32_t index, uint32_t red, uint32_t green, uint32_t blue)
{
...
static esp_err_t LED_STRIP_SPI_ATTR led_strip_spi_refresh(led_strip_t *strip)
{
    led_strip_spi_obj *spi_strip = __containerof(strip, led_strip_spi_obj, base);
...
static esp_err_t LED_STRIP_SPI_ATTR led_strip_spi_clear(led_strip_t *strip)
{
    led_strip_spi_obj *spi_strip = __containerof(strip, led_strip_spi_obj, base);
...
static esp_err_t LED_STRIP_SPI_ATTR led_strip_spi_del(led_strip_t *strip)
{
    led_strip_spi_obj *spi_strip = __containerof(strip, led_strip_spi_obj, base);
    ...
    // Clear the static structure instead of freeing
    memset(spi_strip, 0, sizeof(led_strip_spi_obj));
    return ESP_OK;
}

esp_err_t LED_STRIP_SPI_ATTR led_strip_new_spi_device(const led_strip_config_t *led_config, const led_strip_spi_config_t *spi_config, led_strip_handle_t *ret_strip)
{
    ...
    // Check if the requested LED strip size fits in our static buffer
    uint32_t required_buffer_size = led_config->max_leds * bytes_per_pixel * SPI_BYTES_PER_COLOR_BYTE;
    ESP_GOTO_ON_FALSE(required_buffer_size <= LED_STRIP_STATIC_BUFFER_SIZE, ESP_ERR_NO_MEM, err, TAG,
                      "requested LED strip size (%"PRIu32" bytes) exceeds static buffer size (%d bytes)",
                      required_buffer_size, LED_STRIP_STATIC_BUFFER_SIZE);
    ESP_GOTO_ON_FALSE(led_config->max_leds <= LED_STRIP_MAX_LEDS, ESP_ERR_INVALID_ARG, err, TAG,
                      "requested LED count (%"PRIu32") exceeds maximum (%d)", led_config->max_leds, LED_STRIP_MAX_LEDS);

    // Use static instance instead of dynamic allocation
    spi_strip = &static_spi_strip;
    ...
    ESP_GOTO_ON_ERROR(spi_bus_initialize(spi_strip->spi_host, &spi_bus_cfg, SPI_DMA_DISABLED), err, TAG, "create SPI bus failed");
...
err:
    if (spi_strip) {
        ...
        // Clear the static structure on error
        memset(spi_strip, 0, sizeof(led_strip_spi_obj));
    }
    return ret;
}
